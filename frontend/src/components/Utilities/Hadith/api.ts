...
export const hadithApi = {
  getCategories: async (): Promise<Category[]> => {
    const response = await fetch(`${VITE_API_BASE}/categories/roots/?language=vi`);
  ...
  getHadiths: async (categoryId: number, page: number = 1): Promise<ApiResponse<HadithSummary>> => {
    const response = await fetch(
      `${VITE_API_BASE}/hadeeths/list/?language=vi&category_id=${categoryId}&page=${page}&per_page=12`
    );
  ...
  getHadithDetail: async (hadithId: number): Promise<HadithDetail> => {
    const response = await fetch(`${VITE_API_BASE}/hadeeths/one/?language=vi&id=${hadithId}`);
