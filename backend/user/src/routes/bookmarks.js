...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    const existingBookmark = await Bookmark.findOne({ userId, type, itemId });
    if (existingBookmark) {
...
// Get popular bookmarks (public) — must be before /:id
...
router.post('/bulk', authenticateToken, async (req, res) => {
  try {
    ...
    switch (action) {
      case 'delete':
        result = await Bookmark.deleteMany({ _id: { $in: validIds }, userId });
        break;
      case 'make_public':
...
router.put('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    allowedFields.forEach((field) => {
      if (updateData[field] !== undefined) bookmark[field] = updateData[field];
    });
